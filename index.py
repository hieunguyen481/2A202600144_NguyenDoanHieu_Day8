...
def _split_by_size(
    ...
    for para in paragraphs:
        # Nếu thêm paragraph này vào vượt quá chunk_chars
        if current_chunk and len(current_chunk) + len(para) + 2 > chunk_chars:
            ...
            # Overlap: lấy phần cuối (khoảng 1-2 paragraph) của chunk cũ vào chunk mới
            overlap_text = ""
            temp = current_chunk
            for last_para in reversed(paragraphs):
                if len(overlap_text) + len(last_para) <= overlap_chars:
                    overlap_text = last_para + "\n\n" + overlap_text
                else:
                    break
            current_chunk = overlap_text.rstrip() + "\n\n" + para if overlap_text else para
        else:
            # Thêm paragraph vào chunk
            if current_chunk:
                current_chunk += "\n\n" + para
            else:
                current_chunk = para
...
if __name__ == "__main__":
    ...
    # Bước 3: Build index (yêu cầu OpenAI API key)
    print("\n--- Build Full Index ---")
    try:
        build_index()
        
        # Bước 4: Kiểm tra index
        print("\n--- Kiểm tra Index Quality ---")
        list_chunks(n=5)
